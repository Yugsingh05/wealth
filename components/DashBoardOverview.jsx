...
//   ];
  ...
  const DashBoardOverview = ({ accounts = [], transactions = [] }) => {
    ...
    // Prevent errors if accounts or transactions are empty
    ...
    const currentMonthExpenses = accountTransactions.filter((t) => {
      if (!t.date) return false; // Prevent errors if date is missing
    ...
    const expensesByCategory = currentMonthExpenses.reduce((acc, transaction) => {
      const category = transaction.category || "Uncategorized"; // Avoid undefined categories
      if (!acc[category]) {
        acc[category] = 0;
      }
                  ...
                  <div key={transaction.id} className="flex items-center justify-between">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {transaction.date ? format(new Date(transaction.date), "PP") : "Unknown Date"}
                      </p>
                    </div>
        ...
        <Card>
          ...
          <CardContent className="p-0 pb-5">
            {pieChartData.length === 0 ? (
              ...
              <div className="h-[300px] overflow-auto" style={{ scrollbarWidth: "none" }}>
